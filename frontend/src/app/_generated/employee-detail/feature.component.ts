...
@Component({
  ...
  styles: [
    ...
    .pagination-controls { margin: 0 auto; display: flex; justify-content: center; align-items: center; gap: 0.5rem; padding: 1rem 0; }
    ...
    .empty { color: var(--text-2); font-style: italic; padding: 1rem; text-align: center; }
    `,
  ],
})
export class EmployeeDetailComponent {
  private _employees = signal<Employee[]>([
    { id: 1, name: 'Alice Johnson', department: 'Engineering', position: 'Developer', email: 'alice.j@example.com', phone: '555-0101', date_hired: '2020-01-15' },
    { id: 2, name: 'Bob Smith', department: 'Marketing', position: 'Manager', email: 'bob.s@example.com', phone: '555-0102', date_hired: '2019-03-22' },
    { id: 3, name: 'Charlie Brown', department: 'Sales', position: 'Salesperson', email: 'charlie.b@example.com', phone: '555-0103', date_hired: '2021-06-10' },
    { id: 4, name: 'Dana White', department: 'HR', position: 'Recruiter', email: 'dana.w@example.com', phone: '555-0104', date_hired: '2018-11-05' },
    { id: 5, name: 'Eve Black', department: 'Finance', position: 'Analyst', email: 'eve.b@example.com', phone: '555-0105', date_hired: '2022-02-14' }
  ]);
  ...
  paginationSettings = signal({ currentPage: 1, recordsPerPage: 5 });
  ...
  addEmployee() {
    if (this.newEmployeeName && this.newEmployeeDepartment && this.newEmployeePosition && this.newEmployeeEmail && this.newEmployeePhone && this.newEmployeeDateHired) {
      const newId = Math.max(0, ...this.employees().map(e => e.id)) + 1;
      this._employees.update(list => [...list, {
        id: newId,
  ...
  previousPage() {
    this.paginationSettings.update(settings => ({ ...settings, currentPage: Math.max(1, settings.currentPage - 1) }));
  }

  nextPage() {
    this.paginationSettings.update(settings => ({ ...settings, currentPage: Math.min(this.totalPages(), settings.currentPage + 1) }));
  }
